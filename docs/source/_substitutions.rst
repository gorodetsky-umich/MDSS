.. |mdss| replace:: ``mdss``
.. _mdss: https://github.com/gorodetsky-umich/MDSS.git

.. |OpenMDAO| replace:: ``OpenMDAO``
.. _OpenMDAO: https://github.com/OpenMDAO/OpenMDAO

.. |MPhys| replace:: ``MPhys>=2.0.0``
.. _MPhys: https://github.com/OpenMDAO/mphys

.. |baseClasses| replace:: ``mdolab-baseclasses``
.. _baseClasses: https://github.com/mdolab/baseclasses

.. |IDWarp| replace:: ``IDWarp``
.. _IDWarp: https://github.com/mdolab/idwarp

.. |ADflow| replace:: ``ADflow>=2.12.0``
.. _ADflow: https://github.com/mdolab/adflow

.. |pyOptSparse| replace:: ``pyOptSparse``
.. _pyOptSparse: https://github.com/mdolab/pyoptsparse

.. |TACS| replace:: ``TACS>=3.8.0``
.. _TACS: https://github.com/smdogroup/tacs

.. |funtofem| replace:: ``funtofem>=0.3.9``
.. _funtofem: https://github.com/smdogroup/funtofem